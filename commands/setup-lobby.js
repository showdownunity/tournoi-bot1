...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!participantRole) {
      return interaction.editReply({ content: '❌ Le rôle **Participant** est introuvable ! Vérifie la variable `PARTICIPANT_ROLE_ID` sur Railway.' });
    }
    ...
    const discussionChannel = await guild.channels.create({
      ...
      permissionOverwrites: [
        { id: everyone.id, deny: [PermissionFlagsBits.ViewChannel] },
        {
          ...
          deny: [PermissionFlagsBits.AttachFiles, PermissionFlagsBits.EmbedLinks, PermissionFlagsBits.UseExternalEmojis],
        },
      ],
    });
